// app/service/people/new-born/page.tsx
'use client';

import React, { useEffect, useRef, useState } from 'react';
import Link from 'next/link';
import { motion } from 'framer-motion';
import { Heart, Star, Sparkles, Users, ArrowRight, Zap, Shield, CheckCircle, Camera, Wand2, ChevronLeft, ChevronRight, Sun, Contrast, Layers } from 'lucide-react';

function FloatingStar({ style, size = 16 }: { style: React.CSSProperties; size?: number }) {
  return (
    <motion.div animate={{ y: [-8, 8, -8] }} transition={{ repeat: Infinity, duration: 5 }} className="absolute pointer-events-none" style={style}>
      <Star style={{ width: size, height: size, fill: '#E8352A', opacity: 0.28 }} />
    </motion.div>
  );
}
...
export default function NewBornPage() {
  ...
  const examples = [
    { beforeImage: 'https://images.unsplash.com/photo-1522336572468-97b06e8ef143?w=1200&q=80&auto=format&fit=crop', afterImage: 'https://images.unsplash.com/photo-1522336572468-97b06e8ef143?w=1200&q=80&auto=format&fit=crop' },
    { beforeImage: 'https://images.unsplash.com/photo-1518568814500-bf0f8d125f46?w=1200&q=80&auto=format&fit=crop', afterImage: 'https://images.unsplash.com/photo-1518568814500-bf0f8d125f46?w=1200&q=80&auto=format&fit=crop' },
  ];

  const stats = [
    { value: '5K+', label: 'Babies Retouched' },
    { value: '4.9★', label: 'Parent Rating' },
    { value: '100%', label: 'Gentle Touch' },
    { value: '24h', label: 'Delivery Time' },
  ];

  const relatedServices = [
    { title: 'Maternity Retouching', description: 'Soft, polished edits for maternity portraits and pregnancy sessions.', icon: <Users className="w-8 h-8" />, color: '#E8352A', bg: '#FFF0EE' },
    { title: 'Family Portraits', description: 'Refined color and skin toning for family photo sessions.', icon: <Heart className="w-8 h-8" />, color: '#7C3AED', bg: '#F5F0FF' },
    { title: 'Headshot Retouch', description: 'Professional finishing for brand, actor, and portfolio portraits.', icon: <Star className="w-8 h-8" />, color: '#0EA5E9', bg: '#F0F9FF' },
    { title: 'Lifestyle Editing', description: 'Cinematic finishing for newborn and family lifestyle imagery.', icon: <Sparkles className="w-8 h-8" />, color: '#10B981', bg: '#ECFDF5' },
  ];
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(() => setSliderPosition(s => (s >= 100 ? 50 : s + 0.6)), 60);
  ...
  function changeExample(i: number) { setIndex((i + examples.length) % examples.length); setSliderPosition(50); }
    ...
    <div className="min-h-screen bg-[#0D0D0F] text-white">
      <section className="relative w-full min-h-screen flex flex-col overflow-hidden bg-[#0D0D0F]">
        ...
        <div className="relative z-10 flex-1 flex flex-col lg:grid lg:grid-cols-2 min-h-screen">
          ...
          <div className="flex flex-col justify-center px-8 sm:px-12 lg:px-16 xl:px-24 pt-24 pb-12 lg:py-0">
            ...
            <motion.p className="text-[#A0A0B0] text-base md:text-lg leading-relaxed max-w-md mb-10" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.55, duration: 0.5 }}>Gentle, angel-soft retouching for your newborn's first photos.</motion.p>
          ...
          <motion.div className="relative flex flex-col lg:h-screen" initial={{ opacity: 0, x: 60 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3, duration: 0.8, ease: [0.22, 1, 0.36, 1] }}>
            ...
            <div ref={heroRef} className="relative flex-1 cursor-col-resize select-none overflow-hidden lg:rounded-none rounded-2xl mx-4 lg:mx-0 mt-4 lg:mt-0" style={{ minHeight: 340 }} onMouseMove={handleMove} onMouseEnter={() => setIsHoveringSlider(true)} onMouseLeave={() => { setIsDragging(false); setIsHoveringSlider(false); }} onTouchMove={handleMove} onTouchEnd={() => setIsDragging(false)} onMouseUp={() => setIsDragging(false)}>

              {/* BEFORE — base */}
              <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${examples[index].afterImage})` }} />
              {/* AFTER — clipped */}
              <div className="absolute inset-0 overflow-hidden" style={{ width: `${sliderPosition}%` }}>
                <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${examples[index].beforeImage})`, width: heroRef.current ? `${heroRef.current.offsetWidth}px` : '100%' }} />
              </div>
            ...
            <div className="flex items-center justify-center gap-3 py-5 lg:absolute lg:bottom-24 lg:right-6 lg:flex-col lg:py-0">
              {examples.map((_, i) => (
                <button key={i} onClick={() => changeExample(i)} className={`rounded-full transition-all duration-300 ${index === i ? 'w-8 h-2.5 bg-[#E8352A] shadow-[0_0_8px_rgba(232,53,42,0.7)]' : 'w-2.5 h-2.5 bg-white/25 hover:bg-white/50'}`} />
              ))}
            </div>
          </motion.div>
        </div>
      ...
      <section id="gallery" className="py-16 bg-white text-black">
        <div className="max-w-6xl mx-auto px-6">
          <h2 className="text-3xl font-bold mb-4">See the Magical Difference</h2>
          <div className="grid lg:grid-cols-2 gap-8">
            <div className="rounded-2xl overflow-hidden h-[420px] relative">
              <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${examples[index].afterImage})` }} />
              <div className="absolute inset-0 overflow-hidden" style={{ width: `${sliderPosition}%` }}>
                <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url(${examples[index].beforeImage})` }} />
              </div>
            ...
            <div className="bg-white rounded-2xl border p-6 shadow">
              <h3 className="text-xl font-bold text-[#E8352A] mb-2">Newborn Retouching</h3>
              <p className="text-gray-700 mb-4">Ultra-gentle retouching that preserves your baby's delicate features.</p>
              <div className="text-2xl font-bold mb-4">$0.15 <span className="text-base font-normal text-gray-600">/ image</span></div>
      ...
      <section id="related-services" className="py-20 bg-gradient-to-b from-[#F7F8FA] to-white text-black">
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center max-w-3xl mx-auto mb-12">
            ...
            <p className="text-gray-600 mt-4">Explore related retouching services that complement your newborn photos.</p>
          </div>
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {relatedServices.map((service, i) => (
              <div key={i} className="relative bg-white rounded-[28px] border border-gray-200 p-6 shadow-sm hover:shadow-xl transition-all overflow-hidden group">
                    ...
                    <>
                      ...
                      {openServiceSlug === slug && (
                        <div className="mt-4 rounded-xl border border-gray-100 bg-white p-4 text-sm text-gray-800 shadow-sm">
                          <p className="mb-3">{service.description} Typical turnaround, sample edits, and add-on options.</p>
                          <div className="flex items-center gap-3">
                            ...
                            <Link href={`/service/people/new-born/${slug}`} className="px-3 py-2 rounded-md bg-[#E8352A] text-white">Open full page</Link>
                          </div>
                        </div>
                      )}
                    </>
