...
export default function CheckoutClient({ isLoggedIn }: Props) {
  ...
  const [category, setCategory] = useState<
    'ecommerce' | 'apparel' | 'footwear' | 'furniture'
  >('ecommerce');
  ...
  const basePrice = categoryPrices[category];
  ...
  const handleSubmit = () => {
    ...
    const cartData = {
      ...
      category, // 🔥 this replaces detailing
      qty: orderImages,
      price: basePrice,
      total,
      retouching: "standard",
  ...
  return (
    <section className="bg-white shadow-lg rounded-xl p-6">
      ...
      >
        <option value="ecommerce">E-commerce ($1.50)</option>
        <option value="apparel">Apparel ($1.50)</option>
        <option value="footwear">Footwear ($1.75)</option>
        <option value="furniture">Furniture ($2.25)</option>
      </select>
